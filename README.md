...
| # | 知見 | 確度 | 種別 |
|---|------|:---:|:---:|
| F-5 | **1D-CNN** では複数の拡張が精度を有意に改善(mixup +4.8pt, dtw +4.7pt, oversample +4.1pt ほか。多重比較補正[Holm–Bonferroni]後も α=0.05 で有意) | 高 | 独自 |
| F-6 | **MiniRocket** ではどの拡張も精度を有意に改善しない(効果はモデル依存) | 高 | 独自 |
| F-7 | 拡張効果(1D-CNN)は学習データが中〜低量(25–50%)で相対的に大きい(ただし単調ではなく、フルサイズでも改善が残る) | 中 | 追試 |
| F-16 | 時間軸を持たないデータ(輪郭・スペクトル)を主結果から除いても RQ1 の定性的結論は不変。個々の手法の有意判定は境界で入れ替わる | 高 | 独自 |
...
評価対象は UCR アーカイブから取得した13データセットのうち、**軸が時間である10データセット**です。輪郭やスペクトルを系列化したもの(ArrowHead / Plane / Coffee)は、時間領域の拡張が前提とする「系列が時間に沿って並ぶ」性質を満たさないため主結果から除外し、レポートの付録タブに実験結果ごと収録しています(除外は結果を見たあとの判断であり、除外前の検定結果も付録に併記)。
