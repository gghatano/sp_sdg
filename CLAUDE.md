...
- **実験開始前にコミット**: 実験グリッドを走らせる前にコード変更をコミットし、manifest の git_commit がソースを一意に特定できる状態を保つ
...
- **再現・前処理ノートの維持義務**: 前処理・実験手順・「エイヤッと決めた」判断を変えたら `artifacts/{reproduction_steps,preprocessing_notes,judgment_calls}.yaml`・`deviations.md` を更新する。手順は skill `/reproducibility-notes` に従い、レポート「再現・前処理ノート」タブへ自動反映する
